...
int CNecklace::identifySegment(SSegment *segment, STrackedObject *object, CRawImage *image) {
    ...
    int ID[2];
    SNecklace result[2];
    if (tmp[0].x < tmp[0].m0 || tmp[0].y < tmp[0].m0)
        return -1;
    for (int i = 0; i < 2; i++) {
    ...
    // decision tree to find correct solution
    ...
    segment->angle = 2 * M_PI * (-(float)maxIndex / idSamples - (float)edgeIndex[segIdx] / length / 2.0 + (float)result[segIdx].rotation / length) + atan2(segment->v1, segment->v0);
    while (segment->angle > +M_PI)
        segment->angle -= 2 * M_PI;
    while (segment->angle < -M_PI)
        segment->angle += 2 * M_PI;
    if (debugSegment) {
        ...
        printf("ORIG: ");
        for (int a = 0; a < idSamples; a++)
            printf("%.2f ", signal[segIdx][a]);
        printf("\n");
        for (int a = 0; a < idSamples; a++)
            printf("%.2f ", smooth[segIdx][a]);
        printf("\n");
    }
