...
CRawImage::CRawImage(int wi, int he, int bppi)
    : width(wi), height(he), bpp(bppi), size(bpp * width * height), ownData(true), numSaved(0)
{
...
CRawImage::CRawImage(unsigned char *datai, int wi, int he, int bppi)
    : width(wi), height(he), bpp(bppi), size(bpp * width * height), data(datai), ownData(false), numSaved(0)
{
    memset(header, 0, 122);
    memcpy(header, sth, 122);
    header[18] = width % 256;
